...
function CreateResearchArticle() {
  const [title, setTitle] = useState('');
  const [documentSubtype, setDocumentSubtype] = useState('scientific_and_practical_paper');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { title, document_type: 'research_article', document_subtype: documentSubtype },
        { headers: { Authorization: `Token ${token}` } }
      );
      setDownloadLink(response.data.file); // Используем прямую ссылку из ответа
      setTitle('');
      setDocumentSubtype('scientific_and_practical_paper');
      setErrors({});
    } catch (error) {
      if (error.response && error.response.data) {
        setErrors(error.response.data);
      } else {
        console.error('Произошла ошибка при создании научной статьи!', error);
      }
    } finally {
      setIsLoading(false);
      setMessage('');
      ...
      <Form onSubmit={handleSubmit} isLoading={isLoading} isDisabled={isDisabled}>
        ...
        <div>
          <select value={documentSubtype} onChange={(e) => setDocumentSubtype(e.target.value)} disabled={isLoading}>
            <option value="scientific_and_practical_paper">Научно практическая статья</option>
            <option value="theoretical_paper">Научно теоретическая статья</option>
            <option value="overview_articles">Обзорная статья</option>
          </select>
        </div>
...
export default CreateResearchArticle;